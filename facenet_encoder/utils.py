...
def rescale_img(img):
    """
    Rescale the values of an image from [-1,1] to [0, 255] and permute the channels of the image from [C, 1, H, W] to
    [C, H, W, 1].
    Args:
    ...
    """
    return (img.permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).squeeze().to(torch.uint8)
